...
export default function Navbar() {
  ...
  const accountTo = isAdmin ? '/admin' : user ? '/account' : '/login';
  const accountLabel = isAdmin ? 'Admin' : user ? 'My orders' : 'Sign in';
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200 shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <Link
            to={accountTo}
            className="hidden sm:inline-flex text-sm font-medium text-slate-700 hover:text-brand-700 px-2 py-1.5 rounded-lg hover:bg-slate-100"
          >
            {accountLabel}
          </Link>
          <Link
          ...
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path
                ...
                d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-1.6 4h13.2M9 21a1 1 0 100-2 1 1 0 000 2zm8 0a1 1 0 100-2 1 1 0 000 2z"
              />
            </svg>
            {count > 0 && (
        ...
        <>
          ...
          <div className="lg:hidden absolute left-0 right-0 top-full bg-white border-t border-slate-200 shadow-xl z-40 max-h-[70vh] overflow-y-auto">
            <nav className="px-3 py-3 flex flex-col gap-0.5">
              ...
              <NavLink
                to={accountTo}
                className="px-3 py-3 rounded-lg text-[15px] font-medium text-slate-800 hover:bg-slate-50"
              >
                {accountLabel}
              </NavLink>
            </nav>
